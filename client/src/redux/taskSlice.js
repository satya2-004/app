...
export const addTask = (task, id) => async (dispatch) => {
	...
	const response = await axios.post('http://localhost:5173/task/add', taskData);
	if (response) {
...
export const getAllTasks = (token, id) => async (dispatch) => {
	...
	try {
		const response = await axios.get(
			'http://localhost:5173/task/tasks',
			config
		);
...
export const arrowClick = (item, string) => async () => {
	...
	try {
		let response = await axios.put(
			`http://localhost:5173/task/${taskData.id}`,
			taskData
		);
...
export const deleteItem = (id) => async (dispatch) => {
	let res = await axios.delete(`http://localhost:5173/task/${id}`);
